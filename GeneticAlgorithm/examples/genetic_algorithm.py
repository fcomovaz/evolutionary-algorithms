...
class GeneticAlgorithm:
    ...
    def _validate_parameters(self):
        if not np.issubdtype(type(self.generations), np.integer):
            raise ValueError("Generations must be an integer type.")
        if not np.issubdtype(type(self.population_size), np.integer):
            raise ValueError("Population size must be an integer type.")
        if not np.issubdtype(type(self.chromosome_length), np.integer):
            raise ValueError("Chromosome length must be an integer type.")
        if not np.issubdtype(type(self.mutation_probability), np.floating):
            raise ValueError("Mutation rate must be a floating point type.")
        if not np.issubdtype(type(self.crossover_probability), np.floating):
            raise ValueError("Crossover rate must be a floating point type.")
        if not np.issubdtype(type(self.elitism_count), np.integer):
            raise ValueError("Elitism count must be an integer type.")
        if np.issubdtype(self.chromosome_dtype, np.unsignedinteger):
            if self.chromosome_min_value < 0 or self.chromosome_max_value < 0:
                raise ValueError(
                    "Chromosome min and max values must be non-negative for unsigned integer types."
                )
    ...
    def evolve(
        ...
        for generation in range(self.generations):
            ...
            fitness_scores = self.calculate_fitness(population, fitness_function, custom_args)
        # return population
